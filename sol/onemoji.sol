...
contract ERC721 is ERC165, IERC721 {
    event MintFailed(address miner, uint256 userseed, uint256 tokenId);
    using SafeMath for uint256;
...
contract ERC721Full is ERC721, ERC721Enumerable, ERC721Metadata {
     ...
     * @param nonce nonce ID of the token to be minted
     */
    function _mint(address to, uint256 nonce, uint256 _userSeed) internal returns(bool) {
        require(to != address(0));
        require(totalSupply < 3470, 'Excedend Max Token Supply');
        ...
        uint256 _luck = uint256(keccak256(abi.encodePacked(blockhash( block.number -1), now, nonce, _userSeed))) % 100; //0-99
        uint256 _power = uint256(keccak256(abi.encodePacked(blockhash( block.number -10), now, _luck,  _userSeed))) % 100; //0-99
        uint256 tokenId = uint256(keccak256(abi.encodePacked(blockhash( block.number -100), now, _power, _userSeed))) % 3470; //0-3470
        if(_exists(tokenId)){
            emit MintFailed(to, _userSeed, tokenId);
            return false;
        }
...
contract TradeableERC721Token is ERC721Full, Ownable {
  ...
  uint256 public tokenPrice= 20 ether ;  //price of token to buy
  ...
  {
    require(tx.origin == msg.sender, "onlyHuman!");
    uint256 paidAmount = msg.value;
    require(paidAmount == tokenPrice, "Invalid amount paid");
    uint256 nonce = _getNextTokenId();
    bool success = _mint(msg.sender, nonce, _userSeed);
    _incrementTokenId(); 
...
contract Niftymoji is TradeableERC721Token {
  ...
  uint256 public changePowerPrice = 20 ether; //20 ETH
  uint256 public changeLuckPrice =  20 ether; //20 ETH
